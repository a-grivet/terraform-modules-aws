...
terraform {
  ...
  required_providers {
    ...
    # Required to generate a random password when create_random_password = true
    random = {
      source  = "hashicorp/random" # Official Random provider from HashiCorp
      version = ">= 3.0"           # Minimum Random provider version
      # ">= 3.0" means: Random provider version 3.0 or newer
    }
  }
}
