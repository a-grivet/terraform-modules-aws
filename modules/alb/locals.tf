...
#   1. prefix      : type de ressource (alb, tg) — défini ici, transparent pour l'utilisateur
...
locals {
  ...
  # Noms des ressources du module
  alb_name = "alb-${local.base}${local.label_suffix}"
  tg_name  = "tg-${local.base}${local.label_suffix}"
}
