...
locals {
  ...
  bucket_name = "s3-${local.base}-origin${local.label_suffix}-${data.aws_caller_identity.current.account_id}"

  # Nom pour le tag Name (sans account ID)
  tag_name = "s3-${local.base}-origin${local.label_suffix}"
}
